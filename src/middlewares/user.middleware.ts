...
export default class UserMiddleware {
  public validUser = (req: Request, res: Response, next: NextFunction) => {
  ...
  public validClass = (req: Request, res: Response, next: NextFunction) => {
  ...
  public validLevel = (req: Request, res: Response, next: NextFunction) => {
  ...
  public validPassword = (req: Request, res: Response, next: NextFunction) => {
    const { password } = req.body;
    if (!password) {
      return res.status(BAD_REQUEST).json({ message: '"password" is required' });
    }
    if (typeof password !== 'string') {
      return res.status(UNPROCESSABLE_ENTITY).json({ message: '"password" must be a string' });
    }
